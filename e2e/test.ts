...
const rococo = `wss${colonSlashSlash}rpc.rococo.frequency.xyz`;
...
test('cannot choose sign-up page without providing an endpoint',
  async({page}) => {
  await page.goto('/');
  await expect(page.getByText('Error getting WebSocket endpoint')).toBeVisible();
  await expect(page.getByText('An `endpoint` URL parameter is required.'))
})
